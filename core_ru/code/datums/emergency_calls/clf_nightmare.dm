/datum/emergency_call/clf/nightmare
	name = "Colonial Liberation Front Nightmare(Squad)"
	mob_max = 10
	arrival_message = "'Prepare for your final hour. Expect no mercy. Do you realy wanna have a bad time?'"
	objectives = "Assault the USCM and it's allies to capture the ship for future plunder. Destroy local AI"
	probability = 0
	hostility = TRUE
	home_base = /datum/lazy_template/ert/clf_station

/datum/emergency_call/clf/nightmare/print_backstory(mob/living/carbon/human/H)
	if(ishuman_strict(H))
...
/datum/emergency_call/clf/nightmare/create_member(datum/mind/M, turf/override_spawn_loc)
	...
	if(!leader && HAS_FLAG(H.client.prefs.toggles_ert, PLAY_LEADER) && check_timelock(H.client, JOB_SQUAD_LEADER, time_required_for_job))
		if(H.client.check_whitelist_status(WHITELIST_COMMANDER))
			leader = H
			to_chat(H, SPAN_ROLE_HEADER("You are a Cell Commander of the local resistance group, the Colonial Liberation Front!"))
			arm_equipment(H, /datum/equipment_preset/clf/nightmare/commander, TRUE, TRUE)
		else
			leader = H
			to_chat(H, SPAN_ROLE_HEADER("You are a Cell Leader of the local resistance group, the Colonial Liberation Front!"))
			arm_equipment(H, /datum/equipment_preset/clf/nightmare/leader, TRUE, TRUE)
		...
		if(H.client.check_whitelist_status(WHITELIST_SYNTHETIC_COUNCIL))
			synths++
			...
			arm_equipment(H, /datum/equipment_preset/clf/nightmare/synth/combat, TRUE, TRUE)
		else
			synths++
			to_chat(H, SPAN_ROLE_HEADER("You are a Multi-Purpose Synthetic for the local resistance group, the Colonial Liberation Front!"))
			arm_equipment(H, /datum/equipment_preset/clf/nightmare/synth, TRUE, TRUE)
		...
		arm_equipment(H, /datum/equipment_preset/clf/nightmare/medic, TRUE, TRUE)
		...
		arm_equipment(H, /datum/equipment_preset/clf/nightmare/engineer, TRUE, TRUE)
		...
		arm_equipment(H, /datum/equipment_preset/clf/nightmare/specialist, TRUE, TRUE)
	else
		to_chat(H, SPAN_ROLE_HEADER("You are a Fighter of the local resistance group, the Colonial Liberation Front!"))
		arm_equipment(H, /datum/equipment_preset/clf/nightmare/soldier, TRUE, TRUE)
	print_backstory(H)

	addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(to_chat), H, SPAN_BOLD("Objectives: [objectives]")), 1 SECONDS)


/datum/emergency_call/clf/nightmare/platoon
	name = "Colonial Liberation Front Nightmare (Platoon)"
	mob_min = 8
	mob_max = 35
	probability = 0
	max_engineers = 2
